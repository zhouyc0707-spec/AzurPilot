...
class ActionPointToolbarMixin(WebUIMixinBase):
    ...
    def _render_ap_chart_toolbar(self, current_view, chart_id):
        ...
        # 工具栏只负责布局，视觉（外框、分段控件、按钮）统一由 entry-alas.css
        # 的「统计页统一按钮样式」提供，避免这里再维护一份主题配色。
        # 选择器用 chart_id 占位，保证同一页面多个图表时互不影响。
        put_html(f"""
        <style>
        [style*="--ap-chart-toolbar-{chart_id}"] {{
        ...
        [style*="--ap-chart-segment-{chart_id}"] {{
            display: inline-flex !important;
            width: auto !important;
            max-width: 100% !important;
            margin: 0 !important;
        }}
        [style*="--ap-chart-refresh-{chart_id}"] {{
            margin: 0 !important;
        ...
        @media (max-width: 720px) {{
            ...
            [style*="--ap-chart-toolbar-{chart_id}"] > :first-child {{
            ...
            [style*="--ap-chart-segment-{chart_id}"] .btn-group {{
                width: max-content !important;
                flex-wrap: nowrap !important;
                overflow: visible !important;
            }}
            [style*="--ap-chart-segment-{chart_id}"] .btn {{
                flex: 0 0 auto !important;
                padding: 0 12px !important;
                white-space: nowrap !important;
            }}
            [style*="--ap-chart-segment-{chart_id}"]::-webkit-scrollbar {{
                height: 3px !important;
            }}
            [style*="--ap-chart-segment-{chart_id}"]::-webkit-scrollbar-thumb {{
        ...
        view_buttons = [
            {
                "label": label,
                "value": value,
                "color": "primary" if current_view == value else "off",
            }
        ...
        put_row(
            [
                ...
                ).style(f"--ap-chart-segment-{chart_id}:1;"),
                put_button(
                    t("Gui.Stat.Refresh"),
                    onclick=self._render_ap_chart,
                    color="off",
                ).style(f"--ap-chart-refresh-{chart_id}:1;"),
            ],
            size="auto auto 1fr",
        ).style(f"--ap-chart-toolbar-{chart_id}:1;")
