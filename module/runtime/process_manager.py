...
class ProcessManager:
    ...
    def start_log_queue_handler(self) -> None:
        ...
        # 启动前清掉上一轮的退出信号，否则新线程会立刻退出（上一轮 stop 置过位）。
        # 线程按本轮 run_id / 队列绑定，旧线程自行收尾，无需在此等待它结束。
    ...
    def _thread_log_queue_handler(self, output, process, run_id, queue_lock=None) -> None:
        ...
        if queue_lock is None:
            queue_lock = self._queue_lock
        # 本地定制：除「本轮进程已退出」外，收到停止信号也立即收尾退出，
        # 让 stop() 不必等满轮询周期（stop_log_queue_handler 会 join 这个信号）。
